...
use novaguard\interfaces\SecurityCommandInterface;
use novaguard\parts\Respondable;
use Wujunze\Colors;
use Discord\Discord;
...
class CommandLoader
{
     ...
     */
    public function __construct(NovaGuardInterface $client)
    {
        $this->client = $client;
    ...
    protected function loadDir(string $dir): void
    {
        foreach (scandir($dir) as $file) {
            ...
            if ($file != '.' && $file != '..' && $extension == '.php') {
                ...
                $classnamespace = 'novaguard\\commands\\' . $class;
    ...
    public function loadCommand($cmd)
    {
        ...
        $closure = function (Respondable|Message $respondable, array|Collection $args) use ($cmd) {
            ...
            try {
                ...
                if (\novaguard\Version::TYPE == 'development') {
                    echo $e;
                }
